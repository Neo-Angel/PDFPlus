...
PPTName *nameFromColorSpace(jpeg_decompress_struct *cinfo, PPParser *parser)
{
	//J_COLOR_SPACE
	string *csstr = new string(ColorSpaceStrings[cinfo->out_color_space]);
	PPTName *ret_name = new PPTName(parser, csstr);
	return ret_name;
}


PPImage::PPImage(string path, PPParser *parser)
{
	...
	*/
	...
	PPTName *color_space = nameFromColorSpace(&cinfo, parser);	
	...
	PPTStream *_stream = new PPTStream(parser, file_length);
